...
PHP_FUNCTION(mrss_set_timeout);
PHP_FUNCTION(mrss_get_timeout);
...
static function_entry mrss_functions[] = {
  ...
  PHP_FE (mrss_get_timeout, NULL)
  PHP_FE (mrss_set_timeout, NULL)
...
PHP_FUNCTION (mrss_set_timeout)
{
  ...
  if(zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "l", &timeout) == FAILURE) return;

  if(mrss_set_timeout(timeout)!=MRSS_OK) {
    RETURN_TRUE;
  }  else {
    RETURN_FALSE;
  }
